...
export function isAdmin(ctx) {
  ...
  // kalau ADMIN_ID belum diisi, anggap tidak ada admin -> tolak semua command admin
...
export function errText(e) {
  const msg =
    (e && (e.description || e.message)) ? String(e.description || e.message) : String(e);

  // rapihin beberapa error telegram umum biar gampang dibaca
  if (msg.includes("403")) return "blocked by user (403)";
  if (msg.toLowerCase().includes("bot was blocked")) return "blocked by user";
